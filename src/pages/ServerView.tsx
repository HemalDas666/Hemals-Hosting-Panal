      ...
      <div className="bg-gray-900 border-b border-gray-800 p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex space-x-3 w-full md:w-auto">
          {server.status !== 'online' ? (
            <button onClick={() => handleAction('start')} className="flex-1 md:flex-none justify-center px-4 py-2 bg-green-500/10 hover:bg-green-500/20 text-green-500 font-medium rounded-lg transition-colors border border-green-500/20 flex items-center">
              <Play className="w-4 h-4 mr-2" /> Start
            </button>
          ) : (
            <button onClick={() => handleAction('stop')} className="flex-1 md:flex-none justify-center px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 font-medium rounded-lg transition-colors border border-red-500/20 flex items-center">
              <Square className="w-4 h-4 mr-2" /> Stop
            </button>
          )}
          <button onClick={() => handleAction('restart')} className="flex-1 md:flex-none justify-center px-4 py-2 bg-orange-500/10 hover:bg-orange-500/20 text-orange-500 font-medium rounded-lg transition-colors border border-orange-500/20 flex items-center">
